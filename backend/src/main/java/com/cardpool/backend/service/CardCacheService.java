...
public class CardCacheService {
    ...
    private final ExternalAPIService externalAPIService = new ExternalAPIService(apiBaseUrl);
    private volatile Mono<List<APICard>> cache;
    private volatile Instant cacheExpiry = Instant.MIN;
    private static final Duration TTL = Duration.ofHours(1);

    public Mono<List<APICard>> getAllCards() {
        if (cache == null || Instant.now().isAfter(cacheExpiry)) {
            synchronized (this) {
                if (cache == null || Instant.now().isAfter(cacheExpiry)) {
                    cache = externalAPIService.streamAllCards()
                            .collectList()
                            .cache();
